...
import { StatusBadge } from './TableDataComp'; // Pinjam komponen badge

export default function TableUIUX({ data, isDeadlineClosed }: any) {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // UI/UX tidak ada RMSE, jadi kita urutkan berdasarkan Poin (jika closed) atau Alfabetis (jika open)
      ...
      <table className="w-full text-left border-collapse">
        <thead className="bg-[#172135]/30 text-slate-300 text-xs font-semibold uppercase tracking-[0.12em]">
          <tr className="border-b border-slate-600/30">
            ...
            <th className="py-4 px-4 md:py-5 md:px-8">Nama Kelompok & Kelengkapan Berkas</th>
        ...
        <tbody className="divide-y divide-slate-600/20 text-sm">
          {currentData.length === 0 ? (
            <tr><td colSpan={isDeadlineClosed ? 3 : 2} className="py-16 text-center text-slate-400 italic">Belum ada peserta UI/UX yang terdaftar.</td></tr>
          ) : (
            currentData.map((team, index) => {
              const globalRank = startIndex + index + 1;
              let badgeStyle = 'border-slate-500/30 bg-slate-600/10 text-slate-300';
              if (isDeadlineClosed && globalRank === 1) badgeStyle = 'border-[#ffec1f]/60 bg-[#ffec1f]/15 text-[#ffec1f] drop-shadow-[0_0_8px_rgba(255,236,31,0.3)]';
              else if (isDeadlineClosed && globalRank === 2) badgeStyle = 'border-slate-300/50 bg-slate-400/15 text-slate-200';
              else if (isDeadlineClosed && globalRank === 3) badgeStyle = 'border-amber-600/60 bg-amber-600/15 text-amber-500';
                  ...
                  <td className="py-3 px-3 md:py-4 md:px-8">
                    <div className="flex flex-col gap-1.5 md:gap-2">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 md:gap-2">
                        ...
                        <StatusBadge isUploaded={team.has_mockup} label="Mockup" />
                        <StatusBadge isUploaded={team.has_video} label="Video" />
                        <StatusBadge isUploaded={team.has_prototype} label="Prototype" />
